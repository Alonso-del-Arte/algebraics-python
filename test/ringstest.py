import random
import unittest

import src.numth
from src.rings import IntegerRing
from src.rings import QuadraticRing

class MockRing(IntegerRing) :
    
    def max_degree(self) :
        return 25

class TestIntegerRing(unittest.TestCase) :
    
    def test_str(self) :
        instance = IntegerRing()
        expected = "Z"
        actual = instance.__str__()
        self.assertEqual(expected, actual)
    
    def test_HTML_str(self) :
        instance = IntegerRing()
        expected = "<b>Z</b>"
        actual = instance.HTML_str()
        self.assertEqual(expected, actual)
        
    def test_HTML_blackboardbold_str(self) :
        instance = IntegerRing()
        expectedA = "&#x2124;"
        expectedB = "&#8484;"
        actual = instance.HTML_blackboardbold_str()
        message = actual + " should be " + expectedA + " or " + expectedB
        assert actual == expectedA or actual == expectedB, message
        
    def test_TeX_str(self) :
        instance = IntegerRing()
        expected = "\\textbf Z"
        actual = instance.TeX_str()
        self.assertEqual(expected, actual)
        
    def test_TeX_blackboardbold_str(self) :
        instance = IntegerRing()
        expected = "\\mathbb Z"
        actual = instance.TeX_blackboardbold_str()
        self.assertEqual(expected, actual)
        
    def test_equals(self) :
        expected = IntegerRing()
        actual = IntegerRing()
        self.assertEqual(expected, actual)
        
    def test_not_equals(self) :
        unexpected = IntegerRing()
        actual = MockRing()
        self.assertNotEqual(unexpected, actual)
    
    def test_max_degree(self) :
        instance = IntegerRing()
        expected = 1
        actual = instance.max_degree()
        message = "Reckoning degree of Z"
        self.assertEqual(actual, expected, message)
        
    def test_is_purely_real(self) :
        instance = IntegerRing()
        assert instance.is_purely_real(), "Z is purely real"
        
    def test_discriminant(self) :
        instance = IntegerRing()
        expected = 1
        actual = instance.discriminant()
        message = "Reckoning discriminant of Z"
        self.assertEqual(actual, expected, message)
    
class TestQuadraticRing(unittest.TestCase) :
    
    USUAL_NUMBER_OF_RINGS = 10
    
    def test_str_imag_d_1_mod_4(self) :
        d = -4 * random.randrange(2, 1000) + 1
        rings_so_far = 0
        while rings_so_far < self.USUAL_NUMBER_OF_RINGS :
            if src.numth.squarefree(d) :
                rings_so_far += 1
                instance = QuadraticRing(d)
                expected = f"O_Q(\u221A\u2212{-d})"
                actual = instance.__str__()
                self.assertEqual(actual, expected)
            d -= 4
    
    def test_str_Eisenstein(self) :
        instance = QuadraticRing(-3)
        expected = "Z[\u03C9]"
        actual = instance.__str__()
        self.assertEqual(actual, expected)
        
    def test_str_real_d_1_mod_4(self) :
        d = 4 * random.randrange(1, 1000) + 1
        rings_so_far = 0
        while rings_so_far < self.USUAL_NUMBER_OF_RINGS :
            if src.numth.squarefree(d) :
                rings_so_far += 1
                instance = QuadraticRing(d)
                expected = f"O_Q(\u221A{d})"
                actual = instance.__str__()
                self.assertEqual(actual, expected)
            d += 4
            
    def test_str_Z_phi(self) :
        instance = QuadraticRing(5)
        expected = "Z[\u03C6]"
        actual = instance.__str__()
        self.assertEqual(actual, expected)
    
    def test_str_imag_d_2_mod_4(self) :
        d = -4 * random.randrange(1, 1000) + 2
        rings_so_far = 0
        while rings_so_far < self.USUAL_NUMBER_OF_RINGS :
            if src.numth.squarefree(d) :
                rings_so_far += 1
                instance = QuadraticRing(d)
                expected = f"Z[\u221A\u2212{-d}]"
                actual = instance.__str__()
                self.assertEqual(actual, expected)
            d -= 4
        
    def test_str_real_d_2_mod_4(self) :
        d = 4 * random.randrange(1, 1000) + 2
        rings_so_far = 0
        while rings_so_far < self.USUAL_NUMBER_OF_RINGS :
            if src.numth.squarefree(d) :
                rings_so_far += 1
                instance = QuadraticRing(d)
                expected = f"Z[\u221A{d}]"
                actual = instance.__str__()
                self.assertEqual(actual, expected)
            d += 4
        
    def test_str_imag_d_3_mod_4(self) :
        d = -4 * random.randrange(1, 1000) + 3
        rings_so_far = 0
        while rings_so_far < self.USUAL_NUMBER_OF_RINGS :
            if src.numth.squarefree(d) :
                rings_so_far += 1
                instance = QuadraticRing(d)
                expected = f"Z[\u221A\u2212{-d}]"
                actual = instance.__str__()
                self.assertEqual(actual, expected)
            d -= 4
        
    def test_str_Gaussian(self) :
        instance = QuadraticRing(-1)
        expected = "Z[i]"
        actual = instance.__str__()
        self.assertEqual(actual, expected)
    
    def test_str_real_d_3_mod_4(self) :
        d = 4 * random.randrange(1, 1000) + 3
        rings_so_far = 0
        while rings_so_far < self.USUAL_NUMBER_OF_RINGS :
            if src.numth.squarefree(d) :
                rings_so_far += 1
                instance = QuadraticRing(d)
                expected = f"Z[\u221A{d}]"
                actual = instance.__str__()
                self.assertEqual(actual, expected)
            d += 4
        
    def test_TeX_str_imag_d_1_mod_4(self) :
        d = -4 * random.randrange(2, 1000) + 1
        rings_so_far = 0
        while rings_so_far < self.USUAL_NUMBER_OF_RINGS :
            if src.numth.squarefree(d) :
                rings_so_far += 1
                instance = QuadraticRing(d)
                expected = "\\mathcal O_{\\textbf Q(\\sqrt{" + str(d) + "})}"
                actual = instance.TeX_str()
                self.assertEqual(actual, expected)
            d -= 4
    
    def test_TeX_str_Eisenstein(self) :
        instance = QuadraticRing(-3)
        expected = "\\textbf Z[\\omega]"
        actual = instance.TeX_str()
        self.assertEqual(actual, expected)
        
    def test_TeX_str_real_d_1_mod_4(self) :
        d = 4 * random.randrange(1, 1000) + 1
        rings_so_far = 0
        while rings_so_far < self.USUAL_NUMBER_OF_RINGS :
            if src.numth.squarefree(d) :
                rings_so_far += 1
                instance = QuadraticRing(d)
                expected = "\\mathcal O_{\\textbf Q(\\sqrt{" + str(d) + "})}"
                actual = instance.TeX_str()
                self.assertEqual(actual, expected)
            d += 4
            
    def test_TeX_str_Z_phi(self) :
        instance = QuadraticRing(5)
        expected = "\\textbf Z[\\phi]"
        actual = instance.TeX_str()
        self.assertEqual(actual, expected)
    
    def test_TeX_str_imag_d_2_mod_4(self) :
        d = -4 * random.randrange(1, 1000) + 2
        rings_so_far = 0
        while rings_so_far < self.USUAL_NUMBER_OF_RINGS :
            if src.numth.squarefree(d) :
                rings_so_far += 1
                instance = QuadraticRing(d)
                expected = "\\textbf Z[\\sqrt{" + str(d) + "}]"
                actual = instance.TeX_str()
                self.assertEqual(actual, expected)
            d -= 4
        
    def test_TeX_str_real_d_2_mod_4(self) :
        d = 4 * random.randrange(1, 1000) + 2
        rings_so_far = 0
        while rings_so_far < self.USUAL_NUMBER_OF_RINGS :
            if src.numth.squarefree(d) :
                rings_so_far += 1
                instance = QuadraticRing(d)
                expected = "\\textbf Z[\\sqrt{" + str(d) + "}]"
                actual = instance.TeX_str()
                self.assertEqual(actual, expected)
            d += 4
        
    def test_TeX_str_imag_d_3_mod_4(self) :
        d = -4 * random.randrange(1, 1000) + 3
        rings_so_far = 0
        while rings_so_far < self.USUAL_NUMBER_OF_RINGS :
            if src.numth.squarefree(d) :
                rings_so_far += 1
                instance = QuadraticRing(d)
                expected = "\\textbf Z[\\sqrt{" + str(d) + "}]"
                actual = instance.TeX_str()
                self.assertEqual(actual, expected)
            d -= 4
        
    def test_TeX_str_real_d_3_mod_4(self) :
        d = 4 * random.randrange(1, 1000) + 3
        rings_so_far = 0
        while rings_so_far < self.USUAL_NUMBER_OF_RINGS :
            if src.numth.squarefree(d) :
                rings_so_far += 1
                instance = QuadraticRing(d)
                expected = "\\textbf Z[\\sqrt{" + str(d) + "}]"
                actual = instance.TeX_str()
                self.assertEqual(actual, expected)
            d += 4
        
    def test_TeX_str_Gaussian(self) :
        instance = QuadraticRing(-1)
        expected = "\\textbf Z[i]"
        actual = instance.TeX_str()
        self.assertEqual(actual, expected)
    
    def test_TeX_blackboardbold_str_imag_d_1_mod_4(self) :
        d = -4 * random.randrange(2, 1000) + 1
        rings_so_far = 0
        while rings_so_far < self.USUAL_NUMBER_OF_RINGS :
            if src.numth.squarefree(d) :
                rings_so_far += 1
                instance = QuadraticRing(d)
                expected = "\\mathcal O_{\\mathbb Q(\\sqrt{" + str(d) + "})}"
                actual = instance.TeX_blackboardbold_str()
                self.assertEqual(actual, expected)
            d -= 4
    
    def test_TeX_blackboardbold_str_Eisenstein(self) :
        instance = QuadraticRing(-3)
        expected = "\\mathbb Z[\\omega]"
        actual = instance.TeX_blackboardbold_str()
        self.assertEqual(actual, expected)
        
    def test_TeX_blackboardbold_str_real_d_1_mod_4(self) :
        d = 4 * random.randrange(1, 1000) + 1
        rings_so_far = 0
        while rings_so_far < self.USUAL_NUMBER_OF_RINGS :
            if src.numth.squarefree(d) :
                rings_so_far += 1
                instance = QuadraticRing(d)
                expected = "\\mathcal O_{\\mathbb Q(\\sqrt{" + str(d) + "})}"
                actual = instance.TeX_blackboardbold_str()
                self.assertEqual(actual, expected)
            d += 4
            
    def test_TeX_blackboardbold_str_Z_phi(self) :
        instance = QuadraticRing(5)
        expected = "\\mathbb Z[\\phi]"
        actual = instance.TeX_blackboardbold_str()
        self.assertEqual(actual, expected)
    
    def test_TeX_blackboardbold_str_imag_d_2_mod_4(self) :
        d = -4 * random.randrange(1, 1000) + 2
        rings_so_far = 0
        while rings_so_far < self.USUAL_NUMBER_OF_RINGS :
            if src.numth.squarefree(d) :
                rings_so_far += 1
                instance = QuadraticRing(d)
                expected = "\\mathbb Z[\\sqrt{" + str(d) + "}]"
                actual = instance.TeX_blackboardbold_str()
                self.assertEqual(actual, expected)
            d -= 4
        
    def test_TeX_blackboardbold_str_real_d_2_mod_4(self) :
        d = 4 * random.randrange(1, 1000) + 2
        rings_so_far = 0
        while rings_so_far < self.USUAL_NUMBER_OF_RINGS :
            if src.numth.squarefree(d) :
                rings_so_far += 1
                instance = QuadraticRing(d)
                expected = "\\mathbb Z[\\sqrt{" + str(d) + "}]"
                actual = instance.TeX_blackboardbold_str()
                self.assertEqual(actual, expected)
            d += 4
        
    def test_TeX_blackboardbold_str_imag_d_3_mod_4(self) :
        d = -4 * random.randrange(1, 1000) + 3
        rings_so_far = 0
        while rings_so_far < self.USUAL_NUMBER_OF_RINGS :
            if src.numth.squarefree(d) :
                rings_so_far += 1
                instance = QuadraticRing(d)
                expected = "\\mathbb Z[\\sqrt{" + str(d) + "}]"
                actual = instance.TeX_blackboardbold_str()
                self.assertEqual(actual, expected)
            d -= 4
        
    def test_TeX_blackboardbold_str_Gaussian(self) :
        instance = QuadraticRing(-1)
        expected = "\\mathbb Z[i]"
        actual = instance.TeX_blackboardbold_str()
        self.assertEqual(actual, expected)
    
    def test_TeX_blackboardbold_str_real_d_3_mod_4(self) :
        d = 4 * random.randrange(1, 1000) + 3
        rings_so_far = 0
        while rings_so_far < self.USUAL_NUMBER_OF_RINGS :
            if src.numth.squarefree(d) :
                rings_so_far += 1
                instance = QuadraticRing(d)
                expected = "\\mathbb Z[\\sqrt{" + str(d) + "}]"
                actual = instance.TeX_blackboardbold_str()
                self.assertEqual(actual, expected)
            d += 4
        
    def test_HTML_str_imag_d_1_mod_4(self) :
        d = -4 * random.randrange(2, 1000) + 1
        rings_so_far = 0
        while rings_so_far < self.USUAL_NUMBER_OF_RINGS :
            if src.numth.squarefree(d) :
                rings_so_far += 1
                instance = QuadraticRing(d)
                expected = f"<i>O</i><sub><b>Q</b>(&radic;&minus;{-d})</sub>"
                actual = instance.HTML_str()
                self.assertEqual(actual, expected)
            d -= 4
    
    def test_HTML_str_Eisenstein(self) :
        instance = QuadraticRing(-3)
        expected = "<b>Z</b>[&omega;]"
        actual = instance.HTML_str()
        self.assertEqual(actual, expected)
        
    def test_HTML_str_real_d_1_mod_4(self) :
        d = 4 * random.randrange(1, 1000) + 1
        rings_so_far = 0
        while rings_so_far < self.USUAL_NUMBER_OF_RINGS :
            if src.numth.squarefree(d) :
                rings_so_far += 1
                instance = QuadraticRing(d)
                expected = f"<i>O</i><sub><b>Q</b>(&radic;{d})</sub>"
                actual = instance.HTML_str()
                self.assertEqual(actual, expected)
            d += 4
            
    def test_HTML_str_Z_phi(self) :
        instance = QuadraticRing(5)
        expected = "<b>Z</b>[&phi;]"
        actual = instance.HTML_str()
        self.assertEqual(actual, expected)
    
    def test_HTML_str_imag_d_2_mod_4(self) :
        d = -4 * random.randrange(1, 1000) + 2
        rings_so_far = 0
        while rings_so_far < self.USUAL_NUMBER_OF_RINGS :
            if src.numth.squarefree(d) :
                rings_so_far += 1
                instance = QuadraticRing(d)
                expected = f"<b>Z</b>[&radic;&minus;{-d}]"
                actual = instance.HTML_str()
                self.assertEqual(actual, expected)
            d -= 4
        
    def test_HTML_str_real_d_2_mod_4(self) :
        d = 4 * random.randrange(1, 1000) + 2
        rings_so_far = 0
        while rings_so_far < self.USUAL_NUMBER_OF_RINGS :
            if src.numth.squarefree(d) :
                rings_so_far += 1
                instance = QuadraticRing(d)
                expected = f"<b>Z</b>[&radic;{d}]"
                actual = instance.HTML_str()
                self.assertEqual(actual, expected)
            d += 4
        
    def test_HTML_str_imag_d_3_mod_4(self) :
        d = -4 * random.randrange(1, 1000) + 3
        rings_so_far = 0
        while rings_so_far < self.USUAL_NUMBER_OF_RINGS :
            if src.numth.squarefree(d) :
                rings_so_far += 1
                instance = QuadraticRing(d)
                expected = f"<b>Z</b>[&radic;&minus;{-d}]"
                actual = instance.HTML_str()
                self.assertEqual(actual, expected)
            d -= 4
        
    def test_HTML_str_Gaussian(self) :
        instance = QuadraticRing(-1)
        expected = "<b>Z</b>[<i>i</i>]"
        actual = instance.HTML_str()
        self.assertEqual(actual, expected)
    
    def test_HTML_str_real_d_3_mod_4(self) :
        d = 4 * random.randrange(1, 1000) + 3
        rings_so_far = 0
        while rings_so_far < self.USUAL_NUMBER_OF_RINGS :
            if src.numth.squarefree(d) :
                rings_so_far += 1
                instance = QuadraticRing(d)
                expected = f"<b>Z</b>[&radic;{d}]"
                actual = instance.HTML_str()
                self.assertEqual(actual, expected)
            d += 4
        
    def test_HTML_blackboardbold_str_imag_d_1_mod_4(self) :
        d = -4 * random.randrange(2, 1000) + 1
        rings_so_far = 0
        while rings_so_far < self.USUAL_NUMBER_OF_RINGS :
            if src.numth.squarefree(d) :
                rings_so_far += 1
                instance = QuadraticRing(d)
                expected = f"<i>O</i><sub>&#x211A;(&radic;&minus;{-d})</sub>"
                actual = instance.HTML_blackboardbold_str()
                self.assertEqual(actual, expected)
            d -= 4
    
    def test_HTML_blackboardbold_str_Eisenstein(self) :
        instance = QuadraticRing(-3)
        expected = "&#x2124;[&omega;]"
        actual = instance.HTML_blackboardbold_str()
        self.assertEqual(actual, expected)
        
if __name__ == '__main__' :
    unittest.main()
